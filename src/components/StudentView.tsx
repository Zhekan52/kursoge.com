...
import {
  ...
  FileText, Clock, CheckCircle, AlertCircle, Play, ArrowLeft, ArrowRight, Bell, X
} from 'lucide-react';
import { SUBJECTS, MONTH_NAMES, MONTH_NAMES_GEN, DAY_NAMES, getWeekDates, formatDate, type StudentNotification } from '../data';

type Tab = 'home' | 'schedule' | 'grades' | 'diary' | 'reports';

function loadNotifications(studentId: string): StudentNotification[] {
  try {
    const stored = localStorage.getItem(`notifications_${studentId}`);
    if (stored) return JSON.parse(stored);
  } catch { /* */ }
  return [];
}
function saveNotifications(studentId: string, notifs: StudentNotification[]) {
  localStorage.setItem(`notifications_${studentId}`, JSON.stringify(notifs));
}
...
export const StudentView: React.FC = () => {
  ...
  // ==================== NOTIFICATIONS ====================
  const [notifications, setNotifications] = useState<StudentNotification[]>(() => loadNotifications(studentId));
  const [showNotifPanel, setShowNotifPanel] = useState(false);
  const [toast, setToast] = useState<string | null>(null);
  const prevGradeIdsRef = useRef<Set<string>>(new Set(myGrades.map(g => g.id)));
  const prevDiarySnapshotRef = useRef<string>(JSON.stringify(diaryEntries.filter(e => e.homework).map(e => `${e.id}:${e.homework}:${e.testId}`)));

  const unreadCount = notifications.filter(n => !n.read).length;

  // Watch for new grades
  useEffect(() => {
    const prevIds = prevGradeIdsRef.current;
    const newGrades = myGrades.filter(g => !prevIds.has(g.id));
    if (newGrades.length > 0 && prevIds.size > 0) {
      const newNotifs: StudentNotification[] = newGrades.map(g => ({
        id: `n${Date.now()}${Math.random().toString(36).slice(2, 6)}`,
        type: 'grade',
        message: `Новая оценка ${g.value} по предмету "${g.subject}"`,
        subject: g.subject,
        date: g.date,
        timestamp: Date.now(),
        read: false,
      ...
      if (newGrades.length === 1) {
        setToast(`Новая оценка: ${newGrades[0].value} (${newGrades[0].subject})`);
      } else {
        setToast(`Получено ${newGrades.length} новых оценок`);
      }
    }
    prevGradeIdsRef.current = new Set(myGrades.map(g => g.id));
  }, [myGrades, studentId]);

  // Watch for homework / test changes
  useEffect(() => {
    const currentSnapshot = JSON.stringify(diaryEntries.filter(e => e.homework || e.testId).map(e => `${e.id}:${e.homework}:${e.testId}`));
    if (currentSnapshot !== prevDiarySnapshotRef.current && prevDiarySnapshotRef.current !== '[]') {
      // Find what changed
      const prevEntries = new Set(prevDiarySnapshotRef.current.split(','));
      const currentEntries = diaryEntries.filter(e => e.homework || e.testId);
      const changed = currentEntries.filter(e => !prevEntries.has(`"${e.id}:${e.homework}:${e.testId}"`));

      if (changed.length > 0 && changed.length <= 5) {
        const newNotifs: StudentNotification[] = changed.map(e => ({
          id: `n${Date.now()}${Math.random().toString(36).slice(2, 6)}`,
          type: e.testId ? 'test' : 'homework',
          message: e.testId
            ? `Назначен тест по "${e.subject}"`
            : `Новое ДЗ по "${e.subject}": ${e.homework.slice(0, 50)}`,
          subject: e.subject,
          date: e.date,
          timestamp: Date.now(),
          read: false,
        ...
        setToast(changed.length === 1 ? newNotifs[0].message : `${changed.length} обновлений в дневнике`);
      }
    }
    prevDiarySnapshotRef.current = currentSnapshot;
  }, [diaryEntries, studentId]);
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                {/* Notification Panel */}
                {showNotifPanel && (
                  <div className="absolute right-0 top-12 w-80 max-h-[70vh] bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 animate-scaleIn overflow-hidden">
                    <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                      <h3 className="font-semibold text-gray-900 text-sm">Уведомления</h3>
                      <div className="flex items-center gap-1">
                        {unreadCount > 0 && (
                          <button onClick={markAllRead} className="text-[10px] text-primary-600 hover:underline px-2 py-1">
                            Прочитать все
                          </button>
                        )}
                        {notifications.length > 0 && (
                          <button onClick={clearNotifications} className="text-[10px] text-gray-400 hover:text-red-500 px-2 py-1">
                            Очистить
                          </button>
                        )}
                        <button onClick={() => setShowNotifPanel(false)} className="p-1 rounded hover:bg-gray-200">
                          ...
                          <div key={n.id} className={`px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors ${!n.read ? 'bg-primary-50/50' : ''}`}>
                            <div className="flex items-start gap-3">
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${
                                n.type === 'grade' ? 'bg-green-100' :
                                n.type === 'homework' ? 'bg-blue-100' :
                                n.type === 'test' ? 'bg-violet-100' : 'bg-amber-100'
                              }`}>
                                {n.type === 'grade' ? <ClipboardList className="w-4 h-4 text-green-600" /> :
                                 n.type === 'homework' ? <FileText className="w-4 h-4 text-blue-600" /> :
                                 n.type === 'test' ? <Play className="w-4 h-4 text-violet-600" /> :
                                 <AlertCircle className="w-4 h-4 text-amber-600" />}
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm text-gray-900">{n.message}</p>
                                <p className="text-[10px] text-gray-400 mt-0.5">
                                  {new Date(n.timestamp).toLocaleString('ru-RU', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                                </p>
                              </div>
      ...
      {toast && (
        <div className="fixed top-20 right-4 z-50 animate-slideDown">
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 px-4 py-3 flex items-center gap-3 max-w-sm">
            <div className="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center flex-shrink-0">
              <Bell className="w-4 h-4 text-green-600" />
            </div>
            <p className="text-sm text-gray-900 flex-1">{toast}</p>
            <button onClick={() => setToast(null)} className="p-1 rounded hover:bg-gray-100">
              <X className="w-3.5 h-3.5 text-gray-400" />
            </button>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {activeTab === 'grades' && <Grades myGrades={myGrades} />}
        {activeTab === 'diary' && (
          <Diary
            studentId={studentId}
            lessons={lessons}
...
// ==================== GRADES ====================
const Grades: React.FC<{ myGrades: any[] }> = ({ myGrades }) => {
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {SUBJECTS.map(subject => {
                ...
                const avg = data.allGrades.reduce((a, b) => a + b, 0) / data.allGrades.length;
                return (
...
const Diary: React.FC<DiaryProps> = ({
  ...
  useEffect(() => {
    ...
    const isReal = takingTest.entry?.testType === 'real';
    const hasLimit = isReal && takingTest.test.timeLimit > 0;
    const interval = setInterval(() => {
      setTestTimer(prev => {
        const newVal = prev + 1;
        if (hasLimit && newVal >= takingTest.test.timeLimit * 60) {
          finishTest();
        }
        return newVal;
      });
  ...
  const finishTest = useCallback(() => {
    ...
    const attempt = {
      id: `ta${Date.now()}`, studentId, testId: test.id, date: entry.date,
  ...
  if (takingTest && !testFinished) {
    ...
    const isReal = takingTest.entry?.testType === 'real';
    const remaining = isReal && test.timeLimit > 0 ? test.timeLimit * 60 - testTimer : null;
        ...
        <div className="bg-gradient-to-r from-violet-600 to-purple-700 rounded-2xl p-5 text-white mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              {!isReal && <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full mt-1 inline-block">Тренировочный</span>}
            </div>
  ...
  if (takingTest && testFinished && testResult) {
    const isTraining = !testResult.isReal;

    // Show correct answers for training test
                ...
                <div key={q.id} className={`p-4 rounded-2xl border-2 ${isCorrect ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {q.formula && <div className="mt-1 p-2 bg-white/50 rounded-lg font-mono text-sm">{q.formula}</div>}
                      {q.type === 'text' ? (
                        ...
                        <div className="mt-2 space-y-1">
                          {q.options.map((opt: any) => {
                            const selected = q.type === 'single' ? userAnswer === opt.id : (Array.isArray(userAnswer) ? userAnswer.includes(opt.id) : false);
                            return (
                              <div key={opt.id} className={`flex items-center gap-2 text-sm px-2 py-1 rounded ${
                                selected && opt.correct ? 'text-green-700 font-medium bg-green-100' :
                                selected && !opt.correct ? 'text-red-700 font-medium bg-red-100' :
                                !selected && opt.correct ? 'text-green-600 bg-green-50' :
                                'text-gray-500'
                              }`}>
  ...
  // Confirmation dialog
  if (showConfirm) {
    const isTraining = showConfirm.entry?.testType !== 'real';
    return (
      <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-scaleIn">
          <div className="text-center">
            ...
            {isTraining && (
              <p className="text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-1.5 mb-2">Тренировочный — оценка не выставляется</p>
            )}
            {!isTraining && showConfirm.test.timeLimit > 0 && (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {dayLessons.map((lesson: any) => {
                    // Match diary entry by date + subject (without lessonNumber to fix training test bug)
                    const entry = diaryEntries.find((e: any) => e.date === dateStr && e.subject === lesson.subject) ||
                                  diaryEntries.find((e: any) => e.date === dateStr && e.subject === lesson.subject && e.lessonNumber === lesson.lessonNumber);
                                ...
                                <button onClick={() => setShowConfirm({ test: testObj, entry })}
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <div className="text-[10px] text-violet-500">
                                      {testObj.questions.length} вопр.
                                      {entry?.testType !== 'real' && ' · тренировочный'}
                                      {entry?.testType === 'real' && testObj.timeLimit > 0 ? ` · ${testObj.timeLimit} мин` : ''}
                                    </div>
                                  </div>
                                </button>
                              )}
...
const Reports: React.FC<{ myGrades: any[] }> = ({ myGrades }) => {
  const avgBySubject = useMemo(() => {
    ...
    myGrades.forEach(g => { if (!map[g.subject]) map[g.subject] = []; map[g.subject].push(g.value); });
    return Object.entries(map).map(([subject, vals]) => ({ subject, avg: vals.reduce((a, b) => a + b, 0) / vals.length, count: vals.length }));
  }, [myGrades]);
